...
export function DataTableToolbar<TData>({ table }: DataTableToolbarProps<TData>) {
  ...
  const { setPage, searchTerm, setSearchTerm } = useWithholdingActions();
  return (
    <div className="flex items-center justify-between gap-2">
      <div className="flex flex-1 items-center space-x-2">
        <Input
          placeholder={tCommon('table.filter_placeholder', {
            entity: tInvoicing('withholding.plural')
          })}
      ...
      <Button
        ...
        variant="ghost"
        onClick={() => {
          router.push('/buying/new-withholding');
        }}>
        <PackagePlus className="h-6 w-6" />
        {tInvoicing('withholding.add_button_label')}
      </Button>
