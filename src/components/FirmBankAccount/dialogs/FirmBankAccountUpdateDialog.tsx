...
export const BankAccountUpdateDialog: React.FC<BankAccountUpdateDialogProps> = ({
  ...
  const title = (
    <>
      <Landmark />
      <Label className="font-semibold">{tContacts('bank_accounts.update')}</Label>
    </>
  );
  const description = (
    <>
      <Info className="w-12 h-12" />
      <Label className="leading-5">{tContacts('bank_accounts.hints.update_dialog_hint')}</Label>
    </>
  );
    ...
    return (
      <Dialog open={open} onOpenChange={onClose}>
        <DialogContent className={cn('max-w-[35vw]', className)}>
          ...
          <FirmBankAccountForm className="gap-2 px-3 pb-5" />
