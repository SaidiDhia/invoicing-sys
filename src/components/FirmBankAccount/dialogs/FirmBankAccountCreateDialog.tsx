...
import { FirmBankAccountForm } from '../FirmBankAccountForm';
...
export const BankAccountCreateDialog: React.FC<BankAccountCreateDialogProps> = ({
  ...
  const title = (
    <>
      <Landmark />
      <Label className="font-semibold">{tContacts('bank_accounts.new')}</Label>
    </>
  );
  const description = (
    <>
      <Info className="w-12 h-12" />
      <Label className="leading-5">{tContacts('bank_accounts.hints.create_dialog_hint')}</Label>
    </>
  );
  const footer = (
    ...
    return (
      <Dialog open={open} onOpenChange={onClose}>
        <DialogContent className={cn('max-w-[35vw]', className)}>
          ...
          <FirmBankAccountForm className="gap-2 px-3 pb-5" mainByDefault={mainByDefault} />
